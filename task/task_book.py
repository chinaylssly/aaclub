...
def core_execute_book(table,book,category,sbf,is_test=False):
    ...
    for each in book_data:
        ...
        book_id = each.get('book_id')
        is_exists = sbf.add(book_id)

        if is_exists:
            message = u'book_id:%d in table book'%(book_id)
            ...
            if is_test:
                message = json.dumps(dict(book_id=book_id,book=book_name,category=category),ensure_ascii=False)
                table.logger.debug(message)
            else:
                table.insert_table_book(book_id=book_id,book=book_name,category=category)
...
def execute_book(table,queue,sbf,is_test=False):
    ...
    try:
        book = Book(keyword=keyword,logger=table.logger)
...
def task_book(logger,t_n=4,g_sleep=180,g_times=20,e_sleep=10,e_times=30,is_filter=True,is_test=False):
    ...
    query =u'select book_id from book'
...
def test_task_book(is_test=True):
    ...
    logger = log.Logger
    tasks = task_book(t_n=2,logger=logger,is_test=is_test)
    for t in tasks:
        t.start()
    for t in tasks:
        t.join()
