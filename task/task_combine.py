...
def execute_combine(table,queue,is_test=False,is_cover=False):
    ...
    def combine():
        ...
        query = u'select item_id,item from item where book_id =%d order by item_id asc'%(book_id)
        result_dict = table.execute(query=query)
        data = result_dict.get('data')
        for each in data:
            item_id = each.get('item_id')
            item = each.get('item')
            item_name = u'%s-%s.txt'%(item_id,item)
...
def task_combine(logger,t_n=4,g_sleep=180,g_times=1,e_sleep=10,e_times=30,is_filter=True,is_cover=False,is_test=False):
    ...
    def check_table_book_for_combine(self,):
        query = u'select book_id,book,category,is_finish from book where rate =100'
