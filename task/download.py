...
class Download(Root):
    ...
    def __init__(self,item_id=201093,local=None,logger=None,filepath=None,):

        '''
        item_url like:url='https://www.aastory.club/read.php?id=201093'
        '''
        self.item_id = item_id
        url='https://www.aastory.club/read.php?id=%d'%(self.item_id)

        Root.__init__(self,url=url,local=local,logger=logger)
        if filepath is None:
            filepath = u'%d.txt'%(item_id)
    ...
    ##函数返回值为None，表示文件下载失败，返回True，下载完成
        try:
            ...
            if result:
                ...
                ##获取章节内容的url
                content_url=u'https://www.aastory.club/_getcontent.php?id=%s&v=%s'%(self.item_id,v)
...
def test():
    ...
    download = Download(item_id=200985)
    download.download()
