...
def fix_error_file(table,folder,fsize_list,is_test=False):
    ...
    for filepath in filepaths:
        ...
        if (fsize in fsize_list):
            ...
            item_id = filepath.rsplit('\\',1)[-1].split('-',1)[0]
            table.logger.info(json.dumps(dict(filepath=filepath,fsize=fsize,item_id=item_id),ensure_ascii=False))
            if is_test:
                # message = u'filepath:%s,fszie=%s'%(filepath,fsize)
                # table.logger.info(message)
                pass
            else:
                query = u'update item set is_download = 0 where item_id =%s'%(item_id)
                table.execute(query)
                os.remove(filepath)
