...
def core_execute_item(table,sbf,item,book,category,book_id,is_test=False):
    ...
    if is_test:
        message = u'update book=%s set is_finish=%s'%(book,is_finish)
        table.logger.debug(message)
    else:
        table.update_table_book_is_finish(book_id=book_id,is_finish=is_finish)
    ...
    for each in item_data:
        ...
        is_exists = sbf.add(item_id)

        if is_exists:
            message = u'item_id:%d in table item'%(item_id)
            table.logger.debug(message)
            ...
            if is_test:
                message = json.dumps(dict(item_id=item_id,item=item_name,book=book,category=category,
                                        book_id=book_id,is_finish=is_finish),ensure_ascii=False)
                table.logger.debug(message)
            else:
                table.insert_table_item(item_id=item_id,item=item_name,book=book,category=category,
                                            book_id=book_id,is_finish=is_finish)
...
def execute_item(table,sbf,queue,is_test=False):
    ...
    try:
        item = Item(book_id=book_id,logger=table.logger)
...
def task_item(logger,t_n=4,g_sleep=180,g_times=1,e_sleep=10,e_times=30,is_filter=True,is_test=False):
    ...
    query = u'select item_id from item'
...
def test_task_item(is_test=False):

    log = Log(filename='item.log',name=u'item')
    logger = log.Logger
    tasks = task_item(t_n=4,logger=logger,is_test=is_test)
    for t in tasks:
        t.start()
    for t in tasks:
        t.join()
