...
def execute_download(table,queue,logger,is_test=False):
    ...
    filename = u'%d-%s.txt'%(item_id,item)
    filepath = u'%s/%s'%(filefolder,filename)

    download = Download(item_id=item_id,logger=logger,filepath=filepath)
    ...
    if flag:
        if is_test:
            message = u'update table item set is_downlaod =1 where item_id =%d'%(item_id)
            logger.debug(message)
        else:
            table.update_table_item_is_downlaod(item_id=item_id)
    else:
